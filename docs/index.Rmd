...
```{r echo=TRUE}
# frequencia absoluta da va 'historico de diabetes na familia' sem considerar na
freq_abs <- table(dados1$hist_diab_fam); freq_abs

# frequencia absoluta da va 'historico de diabetes na familia' considerando na
...
# funcao para deixar tabela compacta
compacta_tabela <- function(gt_obj) {
  gt_obj |> 
    gt::tab_options(
...
dados1 |> 
  tbl_summary() |> #<<
  # ignorar (apenas para efeitos de visualizacao no slide)
  as_gt() |>        
  compacta_tabela()
``` 
] 
...
dados1 |> 
  tbl_summary(
    ...
    statistic = list(                                               #<<                                    
      all_continuous() ~  c(                                        #<<
        "{mean} ± {sd}", # media e dp p/ vas quantitativas          #<<
        "{median} ({p25}, {p75})", # mediana e 1 e 3 quartis        #<<
...
.pull-left[
```{r t13, paged.print=FALSE, eval=FALSE}
dados1 |> 
  tbl_summary(
    # analisando por grupo de desfecho
    by = insulina,
...
.pull-right[
```{r t13-out, ref.label="t13", out.width="90%", echo=FALSE, message=FALSE}
```
]
...
.pull-left[
```{r t14, paged.print=FALSE, eval=FALSE}
...
.pull-right[
```{r t14-out, ref.label="t14", out.width="90%", echo=FALSE, message=FALSE}
```
]
...
.pull-left[
```{r t15, paged.print=FALSE, eval=FALSE}
dados1 |> 
  tbl_summary(
    # analisando por grupo de desfecho
    by = insulina,
  ...
  # adicionando rótulo para estratificacao
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Usou insulina antes do parto**") |> 
  # adicionando rodape no cabecalho
  modify_footnote_header( #<<
    footnote = "Gestantes que realizaram o pré-natal entre os anos de 2012 e 2015.", #<<
...
.pull-right[
```{r t15-out, ref.label="t15", out.width="90%", echo=FALSE, message=FALSE}
```
]
...
.pull-left[
```{r t16, paged.print=FALSE, eval=FALSE}
dados1 |> 
  tbl_summary(
    # analisando por grupo de desfecho
    by = insulina,
  ...
  # adicionando rodape no corpo da tabela
  modify_footnote_body( #<<
    footnote = "Parentes de primeiro grau.", #<<
    columns = "label", #<<
    rows = variable == "hist_diab_fam" & row_type == "label" #<<
  ) |> #<<
  # ignorar (apenas para efeitos de visualizacao no slide)
  as_gt() |>        
  compacta_tabela()
...
.pull-right[
```{r t16-out, ref.label="t16", out.width="90%", echo=FALSE, message=FALSE}
```
]
...
.pull-left[
```{r t17, paged.print=FALSE, eval=FALSE}
tbl <- dados1 |> 
  tbl_summary(
    # analisando por grupo de desfecho
    by = insulina,
  ...
  # adicionando rodape avulso
  as_gt() |> #<<
  gt::tab_source_note( #<<
    gt::md("Fonte de dados: ambulatório de diabetes gestacional do HCFMUSP.") #<<
  ) |> #<<
  # ignorar (apenas para efeitos de visualizacao no slide)
  compacta_tabela()
...
.pull-right[
```{r t17-out, ref.label="t17", out.width="90%", echo=FALSE, message=FALSE}
```
]
...
dados1 |> 
  dplyr::filter(!is.na(hist_diab_fam)) |> 
  ggplot(aes(x = hist_diab_fam)) #<<
``` 
] 
...
dados1 |> 
  dplyr::filter(!is.na(hist_diab_fam)) |>
  ggplot(aes(x = hist_diab_fam, fill = insulina)) +
    geom_bar() #<<
``` 
] 
...
dados1 |> 
  ...
  ggplot(aes(x = hist_diab_fam, fill = insulina)) +
    ...
    scale_y_continuous(labels = scales::percent) +
    labs( #<<
      x = "Histórico de diabetes na família", #<<
      y = "Frequência", #<<
      fill = "Usou insulina?" #<<
    ) #<<
``` 
...
ggplot(dados2, aes(x = gini, y = espvida)) +
  ...
  ggpubr::stat_cor( #<<
    ...
    label.x = 0.6, label.y = 78, size = 4.5, color = "#ff0000" #<<
  ) #<<
``` 
] 
...
- <font color="#ff0000">Importante!</font> Por padrão, a função `ggsave( )` salva o último gráfico que foi rodado em seu editor ou console.
